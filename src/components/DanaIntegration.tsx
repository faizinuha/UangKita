...
export const DanaIntegration: React.FC<DanaIntegrationProps> = ({ onClose }) => {
  ...
  const sendOtp = () => {
    ...
    alert(`Kode OTP dikirim ke ${phoneNumber}: 123456`);
    setStep('otp');
  };
        ...
        <div className="p-6">
          {!danaAccount?.isConnected ? (
            <div className="space-y-6">
              {step === 'input' ? (
                ...
                <>
                  ...
                  >
                    {isConnecting ? 'Menghubungkan...' : 'Verifikasi & Hubungkan'}
                  </button>

                  {connectionStatus === 'error' && (
                    <div className="text-red-600 text-sm mt-2">Gagal menghubungkan ke Dana.</div>
                  )}

                  {connectionStatus === 'success' && (
                    <div className="text-green-600 text-sm mt-2">Berhasil terhubung ke Dana!</div>
                  )}
                </>
              )}
            </div>
            ...
            <div className="space-y-6">
              ...
              >
                {isSyncing ? 'Menyinkronkan...' : 'Sinkronkan Saldo'}
              </button>
                ...
                <div className="space-y-3">
                  {danaAccount.transactions.map((transaction) => (
                    <div key={transaction.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${transaction.type === 'in' ? 'bg-green-100' : 'bg-red-100'}`}>
                          <span className="text-sm">{transaction.type === 'in' ? '↓' : '↑'}</span>
                        </div>
                        <div>
                      ...
                      <div className="text-right">
                        <p className={`font-semibold text-sm ${transaction.type === 'in' ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <button
                ...
                className="w-full bg-red-50 text-red-600 py-3 rounded-lg hover:bg-red-100"
              >
